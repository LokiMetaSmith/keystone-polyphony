...
- [x] **Adopt UeberDB-like Storage Portability:** While our current KV/CRDT store is effective, implementing an abstraction layer (similar to UeberDB) could allow node operators to persist mesh state in scalable backends like Redis or PostgreSQL instead of just local SQLite/JSON snapshots.
- [x] **Granular Revision History:** Etherpad explicitly stores a sequence of revisions (`pad:$PADID:revs:$REVNUM`). We should consider adding a dedicated CRDT or logging mechanism that specifically tracks incremental code changesets/deltas, enabling agents to "rewind" state similar to Etherpad's timeslider feature.
