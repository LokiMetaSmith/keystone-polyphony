...
def test_ssh_exchange_logic():
    async def run_test():
        ...
        with patch.dict(os.environ, mock_env), \
             patch("pathlib.Path.home", return_value=Path("/tmp/fakehome")), \
             patch("builtins.open", mock_open(read_data=mock_pubkey_content)) \
             as mocked_file, \
             patch("exchange_ssh_keys.LiminalMesh") as MockMesh:
            ...
            with patch("pathlib.Path.exists", return_value=True):
                ...
                try:
                    with patch(
                        "sys.argv",
                        ["exchange-ssh-keys.py", "--duration", "2"]
                    ):
                        await main()
                except SystemExit:
                    pass
