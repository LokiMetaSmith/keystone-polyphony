...
async def test_chat():
    ...
    # Setup Node 1
    mesh1 = LiminalMesh("chat-test-secret", db_path="chat1.db", identity_path="chat1.pem")
    await mesh1.start()
    
    # Setup Node 2
    mesh2 = LiminalMesh("chat-test-secret", db_path="chat2.db", identity_path="chat2.pem")
    await mesh2.start()
    ...
    if len(res1) == 2 and len(res2) == 2:
        ...
        print(f"FAILURE: Sync incomplete. Node 1 size: {len(res1)}, Node 2 size: {len(res2)}")
