import pytest
from scripts.simulate_deterministic_swarm import (
    SwarmState,
    StartTaskMessage,
    SeniorDevIsolate,
    JuniorDevIsolate,
...
def run_single_simulation_to_trace(seed: int) -> list:
    ...
    scheduler.schedule_event(
        0, scheduler.dispatch_message, "SeniorDev", StartTaskMessage(task_id="Task-1")
    )
    scheduler.schedule_event(
        5, scheduler.dispatch_message, "SeniorDev", StartTaskMessage(task_id="Task-2")
    )
    scheduler.schedule_event(
        10, scheduler.dispatch_message, "SeniorDev", StartTaskMessage(task_id="Task-3")
    )
...
def test_swarm_dst_determinism_across_100_runs():
    ...
    for run_idx in range(1, 100):
        ...
        assert (
            current_trace == reference_trace
        ), f"Determinism broken at run {run_idx} for seed {seed}!"

    print(
        f"Successfully verified absolute determinism across 100 runs for seed {seed}."
    )
